#!/usr/bin/env python3
"""Run corrective SFT v2 evaluation with optional targeted gate check."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run corrective SFT v2 evaluation")
    parser.add_argument(
        "--config",
        type=Path,
        default=CONFIGS_DIR / "eval_sft_corrected_v2.yaml",
    )
    parser.add_argument(
        "--targeted-only",
        action="store_true",
...
def main() -> None:
    ...
    print(
        f"Evaluating {model_cfg['name']} ({model_cfg.get('variant', 'sft_corrected_v2')}) "
    ...
    summary = run_llm_evaluation(
        ...
        model_name=model_cfg["name"],
        variant=model_cfg.get("variant", "sft_corrected_v2_QLoRA_BF16"),
