#!/usr/bin/env python3
"""Run multimodal SFT checkpoint evaluation (Stage F)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run multimodal SFT evaluation")
    parser.add_argument(
        "--config",
        type=Path,
...
def main() -> None:
    ...
    output_cfg = cfg.get("eval_output", {"dir": "results/multimodal_sft_eval"})
    inf_cfg = cfg.get(
        "inference",
        {
            "seed": 42,
    ...
    dataset_path = PROJECT_ROOT / eval_cfg.get(
        "dataset",
        "data/processed/vision/multimodal_eval_heldout.jsonl",
    )
    output_dir = PROJECT_ROOT / output_cfg["dir"]
    adapter_path = PROJECT_ROOT / cfg["output"]["checkpoint_dir"] / "final"

    held_out = set(
        eval_cfg.get("held_out_subjects", ["S026", "S027", "S028", "S029", "S030"])
    )
    ...
    print(
        f"Evaluating {model_cfg['name']} + adapter on {len(examples)} examples "
