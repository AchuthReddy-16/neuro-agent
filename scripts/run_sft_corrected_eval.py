#!/usr/bin/env python3
"""Run corrected SFT adapter held-out evaluation."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run corrected SFT model held-out evaluation"
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=CONFIGS_DIR / "eval_sft_corrected.yaml",
        help="Path to corrected SFT evaluation YAML config",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def main() -> None:
    ...
    print(
        f"Evaluating {model_cfg['name']} ({model_cfg.get('variant', 'sft_corrected')}) "
    ...
    summary = run_llm_evaluation(
        ...
        model_name=model_cfg["name"],
        variant=model_cfg.get("variant", "sft_corrected_QLoRA_BF16"),
