#!/usr/bin/env python3
"""Run conservative GRPO RLVR post-training."""
...
from neuro_agent.training.rlvr_trainer import RLVRTrainer


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run GRPO RLVR training")
    parser.add_argument(
        "--config",
        type=Path,
        default=CONFIGS_DIR / "rlvr.yaml",
        help="Path to RLVR YAML config",
    )
...
def main() -> None:
    ...
    trainer = RLVRTrainer(config, PROJECT_ROOT)
