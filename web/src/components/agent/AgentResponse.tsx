  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <section>
        ...
        {answer.answer?.trim() ? (
          <p className="text-[13px] text-primary leading-relaxed">{answer.answer}</p>
        ) : (
      ...
      {/* COMPUTED — tabular, monospace values, left accent rule */}
      {computed.length > 0 && (
        <section className="border-l-2 border-signal-eeg/50 pl-3">
          ...
          <table className="w-full text-[12px]">
            <tbody>
              {computed.map((item) => (
                <tr key={item.label} className="border-b border-default/50 last:border-0">
                  ...
                  >
                    {item.value}
                    {item.unit && <span className="text-muted ml-1 font-sans text-[10px]">{item.unit}</span>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {/* VISUAL — observation cards, only when vision */}
      {visual.length > 0 && (
        <section className="border-l-2 border-signal-vision/50 pl-3">
          <SectionHead tone="visual" aside={visual.some((v) => v.vlm_interpretation || v.observation) ? "Vision context" : "Image refs (VLM off)"}>
            Visual Evidence
          </SectionHead>
          <div className="space-y-2">
            {visual.map((ref) => (
              <button
                key={ref.id}
                type="button"
                onClick={() => focusVisualization(ref.id, ref.tab as Parameters<typeof focusVisualization>[1])}
              ...
              >
                ...
                {!ref.observation && !ref.vlm_interpretation && (
                  <p className="text-[10px] text-muted mt-1">
                    Visual reference linked — VLM interpretation not enabled.
                  </p>
                )}
              </button>
            ))}
      ...
      {answer.route === "TEXT" && visual.length === 0 && (
        <p className="text-[10px] text-muted">No vision pass — routed as TEXT + TOOLS.</p>
      )}

      {/* INTERPRETATION — prose block */}
      {answer.modelInterpretation && (
        <section className="border-l-2 border-accent-secondary/45 pl-3">
          <SectionHead tone="model" aside="LLM synthesis">
            Model Interpretation
          </SectionHead>
          <p className="text-[12px] text-secondary leading-relaxed">{answer.modelInterpretation}</p>
        </section>
      )}

      {settings.showUncertainty && answer.uncertainty && (
        <section>
          <SectionHead tone="warn">Uncertainty / Limitations</SectionHead>
          <p className="text-[11px] text-muted leading-relaxed">{answer.uncertainty}</p>
        </section>
      )}

      {/* Progressive disclosure for secondary sections */}
      <div className="space-y-2 pt-1 border-t border-default/60">
        ...
        {answer.timeline.length > 0 && (
          <Collapsible title="Execution timeline" compact>
