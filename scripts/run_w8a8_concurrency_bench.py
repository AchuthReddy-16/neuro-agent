#!/usr/bin/env python3
"""Stage I.1 — vLLM W8A8 INT8 concurrency and continuous-batching benchmark.

Online OpenAI-compatible server (not offline LLM.generate batch).
Prefix caching OFF. Production checkpoint only. No H.8, no FP8, no bnb, no SLA.
"""
...
RESULTS_DIR = PROJECT_ROOT / "results" / "serving" / "load" / "w8a8_int8"
CMP_PATH = PROJECT_ROOT / "results" / "model_comparison" / "w8a8_int8_concurrency_scaling.json"
...
# Match H.7 serving knobs closely; prefix cache explicitly off.
...
# Same research-agent prompt family as H.2/H.7; padded to ~512 tokens below.
_BASE_PROMPT = (
...
_PAD = (
    "Additional reproducible context for prompt-length matching: "
    "consider mu (8–12 Hz) and beta (13–30 Hz) contralateral sensorimotor cortex, "
    "CSP spatial filters, LDA/SVM classifiers, ICA ocular artifact rejection, "
    "LORETA source maps, trial-averaged ERD/ERS topography, and BCI feedback latency. "
)

HF_BF16_REF = {"decode_tok_per_s": 52.74, "e2e_ms": 1264.5}
HF_BNB_INT8_REF = {"decode_tok_per_s": 12.5, "e2e_ms": 5450.0}
VLLM_BF16_REF = {"decode_tok_per_s": 61.3, "e2e_ms": 1043.8}
W8A8_SINGLE_REF = {"decode_tok_per_s": 134.3, "e2e_ms": 476.5, "quality": 0.843, "model_vram_gb": 4.27}

CONCURRENCY_PLAN = [1, 4, 8, 16]  # 32 gated
TIMED_REQUESTS = {1: 32, 4: 32, 8: 48, 16: 64, 32: 64}
BENCH_PROMPT = ""  # filled at runtime after tokenizer pad
...
def build_bench_prompt() -> tuple[str, int]:
    from transformers import AutoTokenizer

    tok = AutoTokenizer.from_pretrained(str(CKPT), trust_remote_code=False)
    base = _BASE_PROMPT
    # Grow pad until ~512 tokens, then append Response:
    pad = ""
    while True:
        candidate = base + pad + "Response:"
        n = len(tok.encode(candidate, add_special_tokens=True))
...
def prompt_token_count() -> int:
    global BENCH_PROMPT
    BENCH_PROMPT, n = build_bench_prompt()
    return n
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
            ...
            kernel_lines = [
                ...
                if "CutlassInt8" in ln
                or "CompressedTensorsW8A8Int8" in ln
                or "Selected " in ln and "Int8" in ln
            ]
            quant_lines = [
                ln.strip()
                for ln in log_text.splitlines()
                if "quantization" in ln.lower()
                or "compressed-tensors" in ln.lower()
                or "int-quantized" in ln.lower()
            ][:40]
            cfg = {
                "stage": "I.1",
                "timestamp": now_iso(),
                ...
                "enable_lora": False,
                "quantization_cli": None,
                "quantization_note": "Unset so vLLM reads compressed-tensors W8A8 from checkpoint config",
                "dtype": "auto",
                ...
                "kernel_log_lines": kernel_lines[:20],
                "quantization_log_lines": quant_lines,
                ...
                "h7_single_request_reference": W8A8_SINGLE_REF,
            }
            ...
            sat["hf_bf16_single_request_reference"] = HF_BF16_REF
            sat["hf_bnb_int8_single_request_reference"] = HF_BNB_INT8_REF
            sat["vllm_bf16_single_request_reference"] = VLLM_BF16_REF
            sat["w8a8_single_request_reference_h7"] = W8A8_SINGLE_REF
            sat["main_scaling_baseline"] = "W8A8 concurrency=1 from this run"
            ...
            comparison = {
                ...
                "config": {
                    "vllm_version": vllm.__version__,
                    "dtype": "auto",
                    "quantization": "compressed-tensors W8A8 from checkpoint",
                    "kernel_evidence": cfg.get("kernel_log_lines"),
                    "max_model_len": MAX_MODEL_LEN,
                    "gpu_memory_utilization": GPU_MEMORY_UTILIZATION,
                    "tensor_parallel_size": TENSOR_PARALLEL_SIZE,
                    "enforce_eager": ENFORCE_EAGER,
                ...
                "workload": {
                    ...
                    "temperature": TEMPERATURE,
                    "ignore_eos": True,
                    "prompt_family": "H.2/H.7 research-agent EEG scenario padded to ~512 tokens",
                },
                "single_request_references": {
                    "hf_bf16": HF_BF16_REF,
                    "hf_bnb_int8": HF_BNB_INT8_REF,
                    "vllm_bf16": VLLM_BF16_REF,
                    "vllm_w8a8_h7": W8A8_SINGLE_REF,
                },
                "levels": {str(k): v for k, v in levels.items()},
                "saturation": sat,
                "continuous_batching_supported_by_evidence": evidence[
                    "continuous_batching_supported_by_evidence"
                ],
                "verdict": "PASS"
                if levels
                and all(levels[c]["failed_requests"] == 0 for c in levels)
                and evidence.get("continuous_batching_supported_by_evidence")
                else "PARTIAL_PASS",
            }
            save_json(CMP_PATH, comparison)
            print("\nI.1 complete. levels=", list(levels), "verdict=", comparison["verdict"])
            print("I1_DONE")
    finally:
        gpu_poller.stop()
